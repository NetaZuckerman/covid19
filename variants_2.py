...
for sample, sample_mutlist in samples_mutations.items():
    ...
    if not known_variant:  # did not find variant that has 100% mutations in sample
        """
        Choose the variant with most mutations found in sample as the suspected variant.
        calculate number of found mutations out of all mutaitons and out of covered ones.
        """
        ...
        for key, tup in lin_number.items():  # pick variant that has highest muations % but at least 60 %
            ...
            val = tup[0]  # number of the lineage found mutations in sample
